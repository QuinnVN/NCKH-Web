	...
	<div class="page-shell">
		...
		{#if !completed}
			<section class="resume-panel" aria-label="Tiến trình bảng câu hỏi">
				...
				<a class="button button-lime" href="/questionnaire"
		...
		<div class="report-tabs" role="tablist" aria-label="Các chế độ xem hồ sơ">
			...
			>
				Đánh giá AI <span>MINH HỌA</span>
			</button>
		</div>
			...
			{#if !completed && !showSample}
				<section class="empty-profile" aria-label="Chưa có hồ sơ">
					...
					<a class="button button-outline" href="/questionnaire"
						>Đến bảng câu hỏi <span aria-hidden="true">↗</span></a
					>
				</section>
			{:else}
				<section
				...
				>
						...
						<div class="profile-list">
							{#each visibleModel.profileRows as row (row.code)}
								<div class="profile-row">
									<span class="profile-code">{row.code}</span>
									<div>
										<strong>{row.label}</strong><small>{row.detail}</small>
									</div>
									<b>{row.value}</b>
								</div>
							{/each}
					...
					<div class="profile-side">
						<div class="panel match-panel">
							<div class="panel-heading">
								<div>
									<p class="kicker">02 / Điều gì phù hợp?</p>
									<h2>Mức độ phù hợp nghề nghiệp</h2>
								</div>
								<span class="panel-icon blue">⌁</span>
							</div>
							<div class="demo-banner">
								BẢN XEM TRƯỚC MINH HỌA
								<span>Đánh giá AI chưa được kết nối</span>
							</div>
		...
		{:else}
			<section class="final-report" aria-label="Báo cáo đánh giá AI mẫu">
				<div class="report-topline">
					...
					<div class="fit-ring">
						<strong>84%</strong><span>ĐỘ PHÙ HỢP MẪU</span>
					</div>
				</div>
				<div class="report-columns">
					<div>
						<div class="panel observation-panel">
							<div class="panel-heading compact">
								<div>
									<p class="kicker">Tự đánh giá và hành vi quan sát</p>
									<h2>Điều gì sẽ thay đổi sau VR?</h2>
								</div>
							</div>
					...
					<div>
						<div class="panel roadmap-panel">
							<div class="panel-heading compact">
								<div>
									<p class="kicker">Phát triển cá nhân</p>
									<h2>Lộ trình</h2>
								</div>
							</div>
		...
		{#if downloadNotice}
			<p class="notice" role="status">{downloadNotice}</p>
		{/if}
	</div>
</main>
