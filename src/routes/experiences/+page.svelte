...
const recommended = $derived(
	experiences.find((experience) => experience.slug === targetSlug) ??
		experiences[0],
);
...
const supportingExperiences = $derived(
	visibleExperiences.filter(
		(experience) => experience.slug !== recommended.slug,
	),
);

onMount(() => {
	const slug = new URLSearchParams(window.location.search).get("career");
	if (slug && experiences.some((experience) => experience.slug === slug))
		targetSlug = slug;
});
...
<main class="library-page">
	<div class="library-shell">
		<div class="eyebrow-row">
			<p class="eyebrow">03 / TRẢI NGHIỆM NGHỀ NGHIỆP VR</p>
			<span class="step-pill">{experiences.length} NGHỀ NGHIỆP KHẢ DỤNG</span>
		</div>
		<section class="library-intro">
			<div>
				<p class="kicker">Thư viện nghề nghiệp</p>
				<h1>Chọn một nghề để trải nghiệm.</h1>
				<p class="intro-copy">
					Bước vào môi trường làm việc mô phỏng và quan sát cách bạn phản ứng
					trong tình huống thực tế. Đọc chi tiết nhiệm vụ trước khi chọn nghề để
					so sánh.
				</p>
			</div>
			...
			>
				...
				<div class="recommended-content">
					<div class="card-topline">
						<span class="status status-recommended"
							>{statusLabel(recommended.status)} <i></i></span
						><span class="category-label">{recommended.category}</span>
					</div>
					<p class="kicker">Góc nhìn được đề xuất</p>
					<h2>{recommended.title}</h2>
					<div class="duration">
						<span aria-hidden="true">◷</span>{recommended.missionCount}
						nhiệm vụ · {recommended.duration}
					</div>
					<p class="card-description">{recommended.description}</p>
					<div class="observed">
						<span>Yếu tố được quan sát</span>
						<div>
					...
					<span class="card-action"
						>Xem chi tiết nhiệm vụ <span aria-hidden="true">→</span></span
					>
				</div>
			</button>
			<aside class="target-note">
				<span class="note-icon">i</span>
				<div>
					<p class="kicker">Vì sao là nghề này?</p>
					<strong>{recommended.title} là mục tiêu hiện tại của bạn.</strong>
					<p>
						Đề xuất dựa trên sở thích nghề nghiệp bạn đã chọn. Tỷ lệ so sánh
						trong hồ sơ vẫn là dữ liệu minh họa cho đến khi AI được kết nối.
					</p>
				</div>
			</aside>
		</section>
				...
				{#each supportingExperiences as experience (experience.slug)}
					...
					>
						<div class="experience-image">
							...
							<span
								class:locked={experience.status === 'locked'}
								class="status overlay-status"
								>{statusLabel(experience.status)}</span
							>
						</div>
						<div class="experience-body">
							<div class="card-topline">
								<span class="category-label">{experience.category}</span
							...
							<span class="card-action"
								>Xem chi tiết <span aria-hidden="true">→</span></span
							>
						</div>
					</button>
				{/each}
		...
		{:else}
			<section class="empty-results">
				...
				<div>
					<p class="kicker">Không có kết quả khớp</p>
					<h2>Hãy thử tìm kiếm hoặc danh mục khác.</h2>
					<p>Thẻ mục tiêu vẫn hiển thị phía trên để bạn không mất vị trí.</p>
				</div>
				<button
		...
		<footer class="library-footer">
			<a href="/evaluation">← Quay lại kết quả</a
			><span
				>Các tình huống nghề nghiệp được thiết kế để suy ngẫm. Không có trải
				nghiệm nào được khởi chạy từ bản xem trước này.</span
			>
		</footer>
	</div>
</main>
		...
		>
			{#if selected}
				<div class="dialog-head">
					...
					<Dialog.Close class="dialog-close" aria-label="Đóng chi tiết"
						>×</Dialog.Close
					>
				</div>
				<div class="dialog-image"><img src={selected.image} alt=""></div>
				<div class="dialog-meta">
					<span>{selected.missionCount} nhiệm vụ</span
					><span>{selected.duration}</span
					><span
						>{selected.status === 'locked' ? 'Mô-đun đang phát triển' : 'Tổng quan tình huống'}</span
					>
				</div>
				<div class="dialog-columns">
					<div>
						<h3>Trọng tâm nhiệm vụ</h3>
						<ul>
				...
				<div class="dialog-fit">
					<p class="kicker">Vì sao nên so sánh?</p>
					<p>{selected.whyItFits}</p>
				</div>
				<div class="dialog-actions">
					<Dialog.Close class="button button-outline"
						>Quay lại kết quả</Dialog.Close
					><a class="button button-lime" href="/evaluation"
						>Xem hồ sơ <span aria-hidden="true">↗</span></a
					>
				</div>
			{/if}
		</Dialog.Content>
