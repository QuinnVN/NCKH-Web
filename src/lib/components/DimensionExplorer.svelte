...
>
	...
	>
		<div class="flex min-h-0 flex-col pt-12 pb-0 min-[1001px]:min-h-[40rem] min-[1001px]:pb-12">
			...
			<div
				class="grid min-h-72 pt-[3.3rem] min-[1001px]:min-h-80 [&>div]:[grid-area:1/1]"
				aria-live="polite"
			>
				{#key active.id}
					<div
						in:fly={{ y: reduceMotion ? 0 : 12, duration: reduceMotion ? 0 : 360 }}
						out:fade={{ duration: reduceMotion ? 0 : 140 }}
					>
		...
		<div class="flex flex-col items-center pb-8 min-[1001px]:pb-0">
			<div
				class="grid w-full grid-cols-2 gap-3 min-[601px]:grid-cols-3 min-[601px]:gap-[1.3rem]"
				role="tablist"
				aria-label="Các khía cạnh DESMAP"
			>
				{#each dimensions as dimension, index (dimension.id)}
					<button
						...
						tabindex={activeId === dimension.id ? 0 : -1}
						onclick={() => choose(dimension.id)}
