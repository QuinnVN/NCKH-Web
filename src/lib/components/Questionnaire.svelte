			...
			<main class="assessment-main">
				<header class="assessment-header">
					<div>
						...
						<p class="header-caption">Một khoảng dừng để cân nhắc trước hướng đi tiếp theo.</p>
					</div>
					<div class="progress-copy"><span>{progress}%</span><small>{mode === 'career' ? 'SỞ THÍCH NGHỀ NGHIỆP' : mode === 'review' ? 'SẴN SÀNG GỬI' : `${currentIndex + 1} / ${totalQuestionCount} CÂU HỎI`}</small></div>
				</header>
				...
				{#if mode === 'career'}
					...
					<h1 id="career-title">Bạn tò mò về<br /><span>loại công việc nào?</span></h1>
					<p class="intro-copy">Chọn tối đa ba lĩnh vực rộng. Lựa chọn của bạn giúp bài đánh giá có thêm ngữ cảnh khi bạn khám phá các câu hỏi.</p>
						...
						<div class="selection-row">
							<span class="selection-count" aria-live="polite"><span aria-hidden="true">☆</span> {selectedCareerInterests.length} <em>trên 3 đã chọn</em></span>
							<button class="primary-action" type="button" onclick={startAssessment}>Tiếp tục với DESMAP <span aria-hidden="true">→</span></button>
						</div>
					</section>
				{:else if mode === 'questions' && currentQuestion}
					<section class="panel question-panel" aria-labelledby="question-title">
						...
						<p class="instruction" id="question-instruction">Chọn phương án mô tả đúng nhất về bạn.</p>

						<fieldset class="options-fieldset" aria-describedby="question-instruction question-error">
							<legend class="sr-only">Các phương án trả lời cho câu hỏi {currentQuestion.id}</legend>
						...
						<div class="question-footer">
							<button class="quiet-action" type="button" onclick={previousQuestion}><span aria-hidden="true">←</span> Quay lại</button>
							<span class="question-count">{currentIndex + 1} <span>/</span> {totalQuestionCount}</span>
							<button class="primary-action" type="button" onclick={nextQuestion} disabled={!answers[currentQuestion.id]}>{currentIndex === totalQuestionCount - 1 ? 'Xem lại câu trả lời' : 'Câu tiếp theo'} <span aria-hidden="true">→</span></button>
						</div>
					</section>
				{:else}
					...
					<h1 id="review-title">Bạn đã dành không gian<br /><span>cho một hướng đi rõ ràng hơn.</span></h1>
					<p class="intro-copy">Bạn đã trả lời {answeredCount} trên {totalQuestionCount} câu hỏi. Hãy xem lại bất kỳ giai đoạn nào trước khi tạo hồ sơ.</p>
						...
						<div class="review-list">
							{#each questionnaireStages as stage (stage.id)}
								...
								<div class="review-row">
									...
									<div class="review-row-meta"><span>{stageStatus.answered}/{stageStatus.total}</span><button type="button" class="text-action" onclick={() => jumpToStage(stage.id)}>Xem lại</button></div>
								</div>
							{/each}
						</div>
						...
						<div class="question-footer review-footer">
							<button class="quiet-action" type="button" onclick={previousQuestion}><span aria-hidden="true">←</span> Quay lại</button>
							<span class="saved-note" aria-live="polite">{unansweredQuestions.length === 0 ? 'Đã trả lời tất cả câu hỏi' : `Còn ${unansweredQuestions.length} câu cần trả lời`}</span>
							<button class="primary-action" type="button" onclick={submitAssessment} disabled={unansweredQuestions.length > 0}>Hoàn tất đánh giá <span aria-hidden="true">→</span></button>
						</div>
					</section>
				{/if}
