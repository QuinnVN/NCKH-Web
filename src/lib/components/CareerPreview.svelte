...
const careers: Career[] = [
	...
	},
	{
		...
		category: "Phân tích",
		description:
			"Tìm tín hiệu trong dữ liệu phức tạp và chuyển thành những quyết định hữu ích.",
	...
	},
	{
		...
		category: "Tác động",
		description:
			"Giúp tổ chức tạo ra tiến bộ có thể đo lường cho con người và hành tinh.",
...
>
	<div
		class="mx-auto w-[min(100%_-_2rem,90rem)] min-[701px]:w-[min(100%_-_4rem,90rem)]"
	>
		<div
		...
		>
			<span>Thư viện nghề nghiệp</span
			><span>Khám phá một hướng đi, rồi cảm nhận nghề đó.</span>
		</div>
		<div
		...
		>
			...
			<a
				class="mt-[1.7rem] inline-block whitespace-nowrap font-mono text-[.72rem] tracking-[.04em] text-text uppercase no-underline min-[701px]:mt-0 [&>span]:text-[1.2rem] [&>span]:text-lime"
				href="/experiences"
				>Xem toàn bộ thư viện <span aria-hidden="true">↗</span></a
			>
		</div>
		<div
			class="mb-[1.3rem] flex flex-wrap gap-[.6rem]"
			aria-label="Danh mục nghề nghiệp"
		>
				...
				>
					...
					>
						<span
							...
							>{career.title.split(' ').map((word) => word[0]).join('')}</span
						>
					</div>
					<small
						class="font-mono text-[.63rem] tracking-[.1em] text-(--career-color) uppercase"
						>{career.category}</small
					>
					<h3
						class="mt-[.65rem] mb-0 text-[1.4rem] font-[520] tracking-[-.05em]"
					>
						{career.title}
					</h3>
					<p
						class="mt-[.7rem] mb-[1.2rem] text-[.86rem] leading-[1.45] text-muted"
					>
						{career.description}
					</p>
					<span
	...
	<Dialog.Root
		open={selected !== null}
		onOpenChange={(open) => { if (!open) selected = null; }}
	>
		<Dialog.Portal>
			<Dialog.Overlay
			...
			>
				{#if selected}
					...
					>
						<span
							class="text-[4.7rem] font-light tracking-[-.14em] text-(--career-color) [text-shadow:0_0_1.2rem_var(--career-color)]"
							>{selected.title.split(' ').map((word) => word[0]).join('')}</span
						>
					</div>
					<div class="p-[1.8rem_1rem_1rem] min-[701px]:p-[2.7rem_2.3rem]">
						<p
							class="m-0 font-mono text-[.7rem] tracking-[.12em] text-lime uppercase"
						>
							{selected.category}
						...
						<h4
							class="mt-8 mb-[.8rem] font-mono text-[.68rem] tracking-[.08em] uppercase"
						>
						...
						<a
							...
							href="/questionnaire"
							>Xem mức độ phù hợp <span aria-hidden="true">↗</span></a
						>
					</div>
					<Dialog.Close
